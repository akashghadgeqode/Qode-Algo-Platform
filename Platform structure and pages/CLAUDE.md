...
| My Algos | `My Algos.dc.html` | Now the app's dashboard/home — icon-menu sidebar (My Algos/Positions/Orders/Holdings) replaces the old Portfolio+Reports items; content stacks My Algos summary/table, then the former Portfolio (Analytics) content, then a Reports section (renamed from "Recently generated"); market ticker, summary stats (Today/Overall toggle), status tabs (Active/Subscribed/Paused/Expired), algo table, empty state; tweak for hasAlgos |
...
| Algo Detail | `Algo Detail.dc.html` | Individual algo page for Qode Yield Enhancer +; tabs (Performance/Best & Worst/Monthly & Regime/About & Disclosures incl. Strategy details: portfolio type, market, instruments, leverage, strategies used, objective, risk controls, active UIDs, current deployment, inception date), sticky deploy card, five-step Deploy modal (legal, subscribe, configure with margin check, authorise, active), Share, Certificate PDF link |
...
Positions/Orders/Holdings/My Algos/Blogs share a fixed 240px dark-gradient sidebar (logo, icon menu on My Algos/Positions/Orders/Holdings, broker-connected status, SEBI reg footer) + topbar (page title, broker filter, refresh, user avatar) + body (summary cards row + white table card). My Algos is the dashboard/home now — there is no separate Dashboard/Portfolio/Analytics route and no standalone Reports page; the topbar "Dashboard" link has been removed everywhere (My Algos is reached via the "My Algos" topbar link or the sidebar icon menu).
