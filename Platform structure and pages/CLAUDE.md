...
| My Algos | `My Algos.dc.html` | Now the app's dashboard/home — icon-menu sidebar (My Algos/Positions/Orders) replaces the old Portfolio+Reports items; content stacks My Algos summary/table, then the former Portfolio (Analytics) content, then a Reports section (renamed from "Recently generated"); market ticker, summary stats (Today/Overall toggle), status tabs (Active/Subscribed/Paused/Expired), algo table, empty state; tweak for hasAlgos |
